...
export default function PelangganPage() {
  ...
  useEffect(() => {
    authClient.getSession().then(({ data }) => {
      ...
      api.get<any[]>("/admin/users", { headers: { "X-Admin-Id": data.user.id } })
        .then((all) => setCustomers(all.filter((u: any) => u.role === "user")))
        .catch(console.error)
  ...
  const table = useTable({
    data: customers,
    pageSize: 15,
    searchKeys: ["name", "email", "whatsapp" as any],
  });
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          {!loading && <p className="text-sm text-muted-foreground">{customers.length} pelanggan</p>}
        </div>
